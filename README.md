...
| 編號 | 功能 | 狀態 |
|------|------|------|
| P1 | Microsoft Entra ID／展示登入、App roles、伺服器 Session | ✅ |
| P2 | 全台公園／學校地圖選點＋搜尋 | ✅ |
...
| P11 | App 內盤點 AI 助理：Phi-4 本機／Azure 雲端 provider＋1,000 題 RAG；雲端 GPU deployment 待配額與成本確認 | ✅／待部署 |
...
現場手測**不覆蓋**演算法 `DBH_cm`（離線暫存 localStorage，連線同步 Cosmos DB）。

樹表用卡片底色＋斑馬紋，表頭深藍、最後一欄「健康度」；篩選列精簡（例如 `全 16`、`! 1`）。成長視窗只留管線與趨勢圖，不再重複一張表。選點頁已拿掉 ColorLegend。

### 本機示範帳號

未設定 Microsoft Entra ID 時才顯示；正式環境使用機關 Microsoft 工作帳號。
