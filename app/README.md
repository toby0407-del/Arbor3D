...
沒有掃描素材時，可在匯入視窗按「一鍵載入完整模擬素材（DEMO）」測試上傳與快速盤點。24 組既有模擬盤點的四格影像借用逢甲大學實拍照片，以及真實掃描 `20260818092855` 的 Tree_001～Tree_016 Segmentation、橫切面與點雲側視；3D 則使用 900 點示意 PLY。畫面會明確標示來源是逢甲且不是目前地點的現場證據。
