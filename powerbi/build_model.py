...
def main():
    ...
    (root/'model.bim').write_text(json.dumps(model,ensure_ascii=False,indent=2))
    (root/'queries.pq').write_text('// Create a Text parameter named AnalyticsFolder, then create each named query separately.\n\n'+'\n'.join(queries))
    (root/'measures.dax').write_text('\n\n'.join(k+' =\n'+v for k,v in MEASURES.items()))
    Path('fabric/table-schema.json').write_text(json.dumps(schema,indent=2))
    ...
    contract={'$schema':'https://json-schema.org/draft/2020-12/schema','title':'Arbor3D Analytics v1','type':'object','required':['schema_version','tables'],'additionalProperties':False,'properties':{'schema_version':{'const':'1.0'},'tables':{'type':'object','required':list(FIELDS),'additionalProperties':False,'properties':definitions}}}
    Path('analytics/schema.json').write_text(json.dumps(contract,indent=2))
